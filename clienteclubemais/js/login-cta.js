...
$(document).ready(function () {
    ...
    $('#atualizarCadastroRegulamento').click(atualizarRegulamento);
    ...
    if (localStorage.getItem('token') != undefined) {
        $.ajax({
            ...
            success: function (data) {
                ...
                $("input[name='AceitaComunicacao1']").prop('checked', data.aceitaComunicacao);
                $("input[name='AceitaComunicacaoSMS1']").prop('checked', data.aceitaComunicacaoSMS);
...
function salvarDados() {
    ...
    if (validaCampos()) {
        ...
        cliente.aceitaRegulamento = true;
        cliente.aceitaComunicacao = $('input[name="AceitaComunicacao"]').is(':checked');
        cliente.aceitaComunicacaoSMS = $('input[name="AceitaComunicacaoSMS"]').is(':checked');
...
function atualizarRegulamento() {

    if (!$('#txtRegulamento').prop('checked')) {
        $(".txtRegulamento").text('Aceite o regulamento para atualizar');
        return;
    } else {
        salvarRegulamento();
    }
...
function salvarRegulamento() {
    ...
    cliente.Sobrenome = cliente.PrimeiroNome == cliente.Sobrenome ? '' : cliente.Sobrenome;
    cliente.aceitaRegulamento = true;
    cliente.aceitaComunicacao = $('input[name="AceitaComunicacao1"]').is(':checked');
    cliente.aceitaComunicacaoSMS = $('input[name="AceitaComunicacaoSMS1"]').is(':checked');

    if ($('input[name="AceitaComunicacao1"]').is(':checked'))
        $('input[name="AceitaComunicacao"]').prop('checked')
    else
        $('input[name="AceitaComunicacao"]').prop('checked', false)

    if ($('input[name="AceitaComunicacaoSMS1"]').is(':checked'))
        $('input[name="AceitaComunicacaoSMS"]').prop('checked')
    else
        $('input[name="AceitaComunicacaoSMS"]').prop('checked', false)
    ...
    $.ajax({
        ...
        success: function (data) {
            if (data != undefined && data != null) {
                $('#regulamento-cta').hide();
                $('#atualizarDados-cta').show();
            }
        },
        error: function (data) {
            $('#lblErrorModalServer').html(data.responseJSON.error);
